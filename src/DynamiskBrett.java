...
/**
 * Created by Hans Jacob on 21.04.2017.
 */
public class DynamiskBrett extends Main {
    public GraphicsContext gc;
    private Canvas canvas;
    private int rad;
    private int kolonne;
    private int[][] brett;
    private ArrayList<ArrayList<Character>> DynamiskBrett;
    private int celleSTR = 5;
    private int gameSpeed = 40;
    private Color backgroundColor;
    private Color cellColor;
    private int brettSize;
    ...
    //Constructor
    public DynamiskBrett(int rad, int kolonne, GraphicsContext gc, Canvas canvas) {
        this.rad = rad;
        ...
        this.canvas = canvas;
        //lag arraylist
        ArrayList<Integer> contain = new ArrayList<Integer>(Arrays.asList(brettSize));
        draw();

        for (int i = 0; i < brettSize(); i++) {
            DynamiskBrett.add(new ArrayList<Character>(brettSize));
            for (int j = 0; j < brettSize; j++) {
                DynamiskBrett.get(i).add('0');
            }
        }
    }
    ...
    public DynamiskBrett(int rad, int kolonne){
        this.rad = rad;
        this.kolonne = kolonne;
